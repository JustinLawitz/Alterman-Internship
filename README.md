...
---
Overview
A twelve-week infrastructure internship covering network engineering, cloud and infrastructure
monitoring, wireless, PKI, physical infrastructure, and vendor management across a multi-site
construction firm.
Environment scope: three Cisco Meraki parent organisations spanning a headquarters campus, three
regional offices, and multiple construction jobsites; a Microsoft Azure estate of roughly forty
resources; a Nutanix cluster; an internal Microsoft PKI; and a ManageEngine OpManager /
Applications Manager monitoring platform.
Headline outcomes
Area	Outcome
Cloud monitoring	Built Azure monitoring from nothing — Azure Monitor Agent deployed via region-specific Data Collection Rules, seven logical monitor groups, per-group dashboards, and OS-specific threshold profiles
SaaS monitoring	Stood up Microsoft 365 tenant monitoring end to end: Entra app registration, Graph API permissions, monitor configuration, executive dashboard, NOC view
Certificate management	Wrote a custom PowerShell TLS probe; built a certificate monitoring programme with expiry alerting; discovered a domain controller certificate that had been expired for over a year and traced the broken renewal pipeline
Flow monitoring	Deployed NetFlow collection across three Meraki organisations
Automation	Built a webhook/REST integration between the monitoring platform and the ITSM system with automatic ticket creation and resolution
Physical infrastructure	Brought a previously unmonitored UPS fleet onto the network with SNMPv3, migrated it to a dedicated management VLAN, and configured alerting
Wireless	Built a virtualised Cisco Catalyst 9800-CL controller to convert Catalyst access points from DNA to Meraki management mode; produced a reusable VM gold image
Operations	Designed and built a physical NOC display wall driven by a Linux kiosk with automated view rotation
Field work	Two multi-day out-of-state deployments — an office teardown/relocation and a greenfield office build
Documentation	Authored a 44-page monitoring platform operations handbook, a cloud resource inventory, a certificate lifecycle runbook, and multiple network topology diagrams
---
